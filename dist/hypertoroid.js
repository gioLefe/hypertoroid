...
(() => {
  ...
  var AudioController = class {
    ...
    assetsManager = DIContainer.getInstance().resolve(ASSETS_MANAGER_DI);
    audioContext = new AudioContext();
    mainGainNode = this.audioContext.createGain();
    playingSounds = {};
    constructor() {
  ...
  var UILabel = class extends GameObject {
    ...
    textStyle = this.DEFAULT_TEXT_STYLE;
    textFillStyle = "#000";
    textStrokeStyle = "#000";
  ...
  var UIClickableLabel = class extends withEvents(UILabel) {
    ...
    addMouseEnterCallback = (ev) => {
      this.mouseEnterCallbacks.push(ev);
    };
    addMouseLeaveCallback = (ev) => {
  ...
  var UIPanel = class extends GameObject {
    ...
    fillStyle;
    strokeStyle;
    textStyle = MENUNODES_STYLE;
    width = 0;
    height = 0;
    items = [];
    allItemsHeight = 0;
    heightGap = MENUNODES_GAP;
    constructor(ctx) {
      super(ctx);
    }
    init(...args) {
